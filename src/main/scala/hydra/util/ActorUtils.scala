...
object ActorUtils extends LoggingAdapter {
  ...
  /**
    * Returns the last path element if a string, or the one before it the last path
    * element starts with '$'
    * @param ref Should be an actor that's a routee;
    * @return
    */
  def routeeParent(ref: ActorRef): String = {
